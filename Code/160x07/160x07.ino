...
void powermeter () { // Измеритель уровня выхода
  ...
  analogRead(fwdpin);
  fwdpower = (12 * fwdpower + 4 * (analogRead(fwdpin) + 1)) >> 4; // Крутой фильтр для усреднения показаний FWD!!!
  analogRead(revpin);
  revpower = (12 * revpower + 4 * (analogRead(revpin) + 1)) >> 4; // Крутой фильтр для усреднения показаний REV!!!
}
